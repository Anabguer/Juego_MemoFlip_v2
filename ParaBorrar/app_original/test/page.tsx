...
import { getLevelFromJson, getAllLevels, LevelData } from '@/data/levels';
...
export default function TestPage() {
  ...
  // Cargar niveles y cartas disponibles
  useEffect(() => {
    loadAvailableCards();
    getAllLevels().then(setAllLevels);
    getLevelFromJson(selectedLevel).then(setLevelConfig);
  }, [selectedLevel]);
  ...
  useEffect(() => {
    ...
    if (!levelConfig || levelConfig.timeSec === 0) return; // Sin tiempo límite
  ...
  if (!levelConfig) {
    return <div className="flex items-center justify-center h-screen">
      <div className="text-white">Cargando nivel...</div>
    </div>;
  }
