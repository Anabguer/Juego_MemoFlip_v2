        ...
        switch ($action) {
                    ...
                    if ($result['success']) {
                        ...
                        $stmt = $pdo->prepare("
                            ...
                            SET codigo_verificacion = ?, 
                                tiempo_verificacion = NOW(),
                                email_verificado = 0,
                                activo = 0
                            WHERE usuario_aplicacion_key = ?
                        ");
                        $stmt->execute([$codigo, $usuario_key]);
            ...
            case 'verify_code':
                try {
                    ...
                    if (!codigoEsValido($usuario['tiempo_verificacion'], 24)) {
            ...
            case 'login':
                try {
                    ...
                    $stmt = $pdo->prepare("
                        SELECT email_verificado FROM usuarios_aplicaciones 
                        WHERE usuario_aplicacion_key = ?
                    ");
                    $stmt->execute([$usuario_key]);
                    $usuario = $stmt->fetch();
                    
                    if ($usuario && $usuario['email_verificado'] == 0) {
